...
PImage svgToImage(PShape p_shape, float p_width, float p_height, boolean p_doStyle) {
  if (p_shape == null)
  try {
    throw new NullPointerException("`svgToImage(null , p_width, p_height, p_doStyle) won't work.`");
  } 
  catch (NullPointerException e) {
    e.printStackTrace();
  }
